...
def evaluate_and_plot(X_reduced, labels, label_suffix):
    ...
    colors = sns.color_palette("Set1", n_colors=len(unique_labels))
    for idx, label in enumerate(unique_labels):
        plt.scatter(X_reduced[labels == label, 0], X_reduced[labels == label, 1],
                    s=30, color=colors[idx], label=f'Cluster {label}' if label != -1 else 'Noise')
...
def format_p(p):
    return "< 1×10⁻⁴" if p < 1e-4 else f"{p:.2e}"
...
def statistical_testing(df_original, labels):
    ...
    print(df_original.groupby('Cluster')[numeric_cols].mean())
...
# calculating pd patients per cluster
...
significant_features = ['Age', 'Ethnicity', 'SleepQuality', 'Diabetes', 'Depression', 'Stroke',
                         'UPDRS', 'MoCA', 'Tremor', 'Rigidity', 'Bradykinesia',
                         'PosturalInstability', 'Diagnosis']

# (In practice, would parse your real p-values + MI outputs)
clusters_info = {
    0: {'Stroke': 'High', 'Depression': 'High'},
    6: {'SleepQuality': 'Lower'},
    17: {'Tremor': 'Absent', 'PosturalInstability': 'Absent'},
    19: {'Diabetes': 'Higher'},
    20: {'MoCA': 'Lower'}
}

summary_data = []
for cluster_id, features in clusters_info.items():
    important_features = ", ".join([f"{k} ({v})" for k, v in features.items()])
    interpretation = ""
    if cluster_id == 0:
        interpretation = "Stroke-related patients with higher depression rates"
    elif cluster_id == 6:
        interpretation = "Sleep disturbances dominant"
    elif cluster_id == 17:
        interpretation = "Non-motor cluster, absence of tremor and instability"
    elif cluster_id == 19:
        interpretation = "Diabetes-associated Parkinson's subgroup"
    elif cluster_id == 20:
        interpretation = "Cognitive decline cluster (MoCA low)"
    summary_data.append((cluster_id, important_features, interpretation))

cluster_summary = pd.DataFrame(summary_data, columns=["Cluster", "Important Features", "Clinical Interpretation"])
def format_p(p):
    if p < 1e-4:
        return "< 1×10⁻⁴"
    else:
...
def full_analysis(selected_features_only, include_stroke, label_suffix):
    df_original, df_numeric, df_scaled = load_and_preprocess(selected_features_only=selected_features_only, include_stroke=include_stroke)
    ...
    cluster_17 = df_original[df_original['Cluster'] == 17]
    print("\n=== Cluster 17 Special Feature Counts ===")
    print(f"Tremor Total Count: {cluster_17['Tremor'].sum()}")
    print(f"Postural Instability Total Count: {cluster_17['PosturalInstability'].sum()}")
    ...
    features_of_interest = [
        'Age', 'Ethnicity', 'SleepQuality', 'Diabetes', 'Depression', 'Stroke',
        'UPDRS', 'MoCA', 'Tremor', 'Rigidity', 'Bradykinesia', 'PosturalInstability'
    ]
    for feature in features_of_interest:
        if feature in df_original.columns:
            fig, axs = plt.subplots(1, 2, figsize=(14, 5), sharey=True)
            df_Nopd = df_original[df_original['Diagnosis'] == 0]

            sns.histplot(df_Nopd[feature], kde=False, bins=30, color='orange', ax=axs[0], stat='count')
            axs[0].set_title(f'No PD Patients - {feature}')
            axs[0].set_xlabel(f'{feature} Value')
            axs[0].set_ylabel('Patient Count')
            df_pd = df_original[df_original['Diagnosis'] == 1]
            sns.histplot(df_pd[feature], kde=False, bins=30, color='blue', ax=axs[1], stat='count')
            axs[1].set_title(f'Parkinson\'s Patients - {feature}')
            axs[1].set_xlabel(f'{feature} Value')
